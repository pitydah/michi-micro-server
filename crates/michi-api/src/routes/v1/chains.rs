...
pub async fn list_chains_handler(
    ...
    let chains = michi_db::list_chains(&state.db)
        .await
...
pub async fn create_chain_handler(
    ...
    if body.name.trim().is_empty() {
        return Err(v1_error(StatusCode::BAD_REQUEST, "VALIDATION_ERROR", "chain name is required"));
    }
...
pub async fn get_chain_handler(
    ...
    match result {
        ...
        None => Err(v1_error(StatusCode::NOT_FOUND, "NOT_FOUND", &format!("chain not found: {}", id))),
    }
}
...
pub async fn update_chain_handler(
    ...
    let updated = michi_db::update_chain(&state.db, &id, &body)
        ...
        .map_err(|e| v1_error(StatusCode::INTERNAL_SERVER_ERROR, "DATABASE_ERROR", &e.to_string()))?;
    if !updated {
        return Err(v1_error(StatusCode::NOT_FOUND, "NOT_FOUND", &format!("chain not found: {}", id)));
    }
...
pub async fn delete_chain_handler(
    ...
    let deleted = michi_db::delete_chain(&state.db, &id)
        .await
        .map_err(|e| v1_error(StatusCode::INTERNAL_SERVER_ERROR, "DATABASE_ERROR", &e.to_string()))?;
    if !deleted {
        return Err(v1_error(StatusCode::NOT_FOUND, "NOT_FOUND", &format!("chain not found: {}", id)));
...
pub async fn add_link_handler(
    ...
    if body.receiver_id.trim().is_empty() {
        return Err(v1_error(StatusCode::BAD_REQUEST, "VALIDATION_ERROR", "receiver_id is required"));
    }
...
pub async fn update_link_handler(
    ...
    let updated = michi_db::update_chain_link(&state.db, &link_id, &body)
        ...
        .map_err(|e| v1_error(StatusCode::INTERNAL_SERVER_ERROR, "DATABASE_ERROR", &e.to_string()))?;
    if !updated {
        return Err(v1_error(StatusCode::NOT_FOUND, "NOT_FOUND", &format!("link not found: {}", link_id)));
    }
    Ok(Json(serde_json::json!({ "status": "updated" })))
}
...
pub async fn play_chain_handler(
    ...
    let (chain, links) = result.ok_or_else(|| {
        v1_error(StatusCode::NOT_FOUND, "NOT_FOUND", &format!("chain not found: {}", id))
    })?;
        ...
        if let Some(entry) = reg_read.get(&link.receiver_id) {
            if entry.paired && entry.active_session_id.is_none() {
                let _ = state.receiver_manager
                    .start_session(&link.receiver_id, &id.to_string(), "pcm", 48000, 24, 2, 0, 200, link.volume as u32)
                    .await;
            }
            // Set volume per receiver
            if link.muted {
                let _ = state.receiver_manager.set_volume(&link.receiver_id, 0).await;
            } else {
                let _ = state.receiver_manager.set_volume(&link.receiver_id, link.volume as u32).await;
            }
        }
    }
...
pub async fn stop_chain_handler(
    ...
    Ok(Json(serde_json::json!({ "status": "stopped", "chain_id": id })))
}
...
pub async fn chain_volume_handler(
    ...
    for link in &links {
        ...
        if let Some(entry) = reg_read.get(&link.receiver_id) {
            if entry.paired {
                let _ = state.receiver_manager.set_volume(&link.receiver_id, volume as u32).await;
            }
        }
    }

    Ok(Json(serde_json::json!({ "status": "volume_set", "volume": volume })))
}
