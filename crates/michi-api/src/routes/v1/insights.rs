...
pub async fn artist_insights_handler(
    ...
    let decoded = urlencoding::decode(&name).map_err(|_| {
        v1_error(StatusCode::BAD_REQUEST, "INVALID_ARTIST_NAME", "url decode failed")
    })?;
    ...
    let earliest_year: Option<i32> = sqlx::query_scalar(
        "SELECT MIN(year) FROM tracks WHERE artist = ? AND year IS NOT NULL"
    )
        .bind(&artist)
        .fetch_one(&state.db).await.unwrap_or(None);

    let latest_year: Option<i32> = sqlx::query_scalar(
        "SELECT MAX(year) FROM tracks WHERE artist = ? AND year IS NOT NULL"
    )
        .bind(&artist)
        .fetch_one(&state.db).await.unwrap_or(None);

    let total_duration_ms: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(duration_ms), 0) FROM tracks WHERE artist = ?"
    )
        .bind(&artist)
        .fetch_one(&state.db).await.unwrap_or(0);
...
pub async fn album_health_handler(
    ...
    let decoded = urlencoding::decode(&key).map_err(|_| {
        v1_error(StatusCode::BAD_REQUEST, "INVALID_ALBUM_KEY", "url decode failed")
    })?;
    let parts: Vec<&str> = decoded.splitn(2, "|||").collect();
    if parts.len() != 2 {
        return Err(v1_error(StatusCode::BAD_REQUEST, "INVALID_FORMAT", "use artist|||album format"));
    }
    let album_artist = parts[0];
    let album = parts[1];

    let track_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM tracks WHERE album = ? AND album_artist = ?"
    )
    ...
    let issues: Vec<String> = {
        let mut v = Vec::new();
        if missing_cover > 0 { v.push(format!("{} tracks missing cover art", missing_cover)); }
        if missing_year > 0 { v.push(format!("{} tracks missing year", missing_year)); }
        v
    };
