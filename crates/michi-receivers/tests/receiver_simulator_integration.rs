...
fn sim_url() -> String {
    std::env::var("MICHI_RECEIVER_SIM_URL")
...
async fn test_receiver_info_standard_output() {
    ...
    assert_eq!(output.get("connector").and_then(|v| v.as_str()), Some("jack_3_5"));
...
async fn test_receiver_pairing_flow() {
    ...
    // pair/start
    let start = client.pair_start("test-flow").await.expect("pair_start failed");
    ...
    let confirm = client.pair_confirm(&nonce, "test-flow", "tok_rust_integration_test").await.expect("pair_confirm failed");
...
async fn test_receiver_pairing_window_closed_rejected() {
    ...
    let start = client.pair_start("test-reject").await.expect("pair_start failed");
    let nonce = start.nonce.expect("must have nonce");

    // First confirm succeeds
    let confirm = client.pair_confirm(&nonce, "test-reject", "tok_first").await.expect("first confirm failed");
    assert_eq!(confirm.status.as_deref(), Some("paired"));

    // Second confirm on same nonce should fail
    let start2 = client.pair_start("test-reject").await.expect("pair_start should still open new window");
    let nonce2 = start2.nonce.expect("must get new nonce");
    let confirm2 = client.pair_confirm(&nonce2, "test-reject", "tok_second").await.expect("second confirm failed");
...
async fn test_receiver_standard_full_lifecycle() {
    ...
    let device_id = mgr.discover_and_pair(&base_url, "test-lifecycle").await.expect("discover and pair failed");

    // Start session
    let session_id = format!("sess_{}", uuid::Uuid::new_v4());
    let sess_resp = mgr.start_session(&device_id, &session_id, "pcm_s16le", 48000, 16, 2, 55300, 250, 70).await.expect("session_start failed");
    ...
    // Set volume
    let vol = mgr.set_volume(&device_id, 42).await.expect("set_volume failed");
    assert_eq!(vol.volume, Some(42));

    // Stop session
    let stop = mgr.stop_session(&device_id).await.expect("session_stop failed");
...
async fn test_receiver_hifi_full_lifecycle() {
    ...
    let vol = mgr.set_volume(&device_id, 75).await.expect("set_volume failed");
...
async fn test_receiver_errors_unsupported_codec() {
    let mgr = ReceiverSessionManager::new();
    let device_id = mgr.discover_and_pair(&sim_url(), "test-codec").await.expect("pair failed");

    let session_id = format!("sess_err_{}", uuid::Uuid::new_v4());
    let result = mgr.start_session(&device_id, &session_id, "aac", 48000, 16, 2, 55500, 250, 70).await;
...
async fn test_receiver_errors_sample_rate_exceeds() {
    let mgr = ReceiverSessionManager::new();
    let device_id = mgr.discover_and_pair(&sim_url(), "test-sr").await.expect("pair failed");

    let session_id = format!("sess_sr_{}", uuid::Uuid::new_v4());
    let result = mgr.start_session(&device_id, &session_id, "pcm_s16le", 96000, 16, 2, 55600, 250, 70).await;
...
async fn test_receiver_errors_duplicate_session() {
    let mgr = ReceiverSessionManager::new();
    let device_id = mgr.discover_and_pair(&sim_url(), "test-dupe").await.expect("pair failed");

    let session_id = format!("sess_dupe_{}", uuid::Uuid::new_v4());
    let first = mgr.start_session(&device_id, &session_id, "pcm_s16le", 48000, 16, 2, 55700, 250, 70).await;
    assert!(first.is_ok(), "first session should succeed");

    let second = mgr.start_session(&device_id, "sess_dupe_2", "pcm_s16le", 48000, 16, 2, 55701, 250, 70).await;
...
async fn test_receiver_errors_volume_out_of_range() {
    let mgr = ReceiverSessionManager::new();
    let device_id = mgr.discover_and_pair(&sim_url(), "test-vol").await.expect("pair failed");

    let vol = mgr.set_volume(&device_id, 101).await.expect("volume 101 should still succeed (clamped)");
    assert!(vol.volume.unwrap_or(0) <= 100, "volume must be clamped to 100");

    let vol2 = mgr.set_volume(&device_id, 999).await.expect("volume 999 should still succeed");
...
async fn test_receiver_errors_unauthenticated() {
    ...
    let client = ReceiverClient::new(&sim_url());
    let hb = client.heartbeat().await.expect("heartbeat returned ok even without auth?");
...
async fn test_receiver_registry_tracks_state() {
    let mgr = ReceiverSessionManager::new();
    let device_id = mgr.discover_and_pair(&sim_url(), "test-reg").await.expect("pair failed");

    let reg = mgr.registry().await;
    let reg_read = reg.read().await;
    let entry = reg_read.get(&device_id).expect("receiver must be in registry");
    assert!(entry.paired);
