...
pub async fn run_hourly_maintenance(pool: &SqlitePool) -> Result<(), DbError> {
    ...
    sqlx::query("DELETE FROM pairing_qr_codes WHERE expires_at < datetime('now') OR claimed = 1")
        .execute(pool)
        .await
        .ok();
...
pub async fn list_stream_sources(
    pool: &SqlitePool,
) -> Result<Vec<michi_core::StreamSource>, DbError> {
    let rows = sqlx::query(
...
pub async fn delete_stream_source(pool: &SqlitePool, id: &Uuid) -> Result<bool, DbError> {
    let id_s = id.to_string();
    sqlx::query("DELETE FROM podcast_episodes WHERE source_id = ?")
        .bind(&id_s)
        .execute(pool)
        .await?;
    let res = sqlx::query("DELETE FROM stream_sources WHERE id = ?")
        .bind(&id_s)
        .execute(pool)
        .await?;
    Ok(res.rows_affected() > 0)
...
pub async fn update_stream_source_enabled(
    pool: &SqlitePool,
    id: &Uuid,
    enabled: bool,
) -> Result<bool, DbError> {
...
pub async fn list_podcast_episodes(
    pool: &SqlitePool,
    source_id: &Uuid,
) -> Result<Vec<michi_core::PodcastEpisodeDb>, DbError> {
    ...
    let episodes = rows
        ...
        .map(|r| {
            ...
            michi_core::PodcastEpisodeDb {
                ...
                pub_date: pub_date_str.map(|s| s.to_string()),
