...
| **Rooms (Multi-Room)** | `beta` | v1-beta | Agrupación y topología de salas; Snapcast daemon real verificado en CI |
| **Sync Causal** | `stable` | Core | Lamport clocks, epoch precedence ante reinicio, orden causal total en WS |
| **Web UI** | `stable` | Core | SPA autónoma integrada, responsive, i18n 9 idiomas, cache busting dinámico, SW con claims |
| **Docker Packaging** | `stable` | Core | Imagen multi-arch `linux/amd64` y `linux/arm64`, puerto canónico 9090 |
| **CasaOS / ZimaOS** | `beta` | Core | Paquete dist validado en CI; certificación física en hardware pendiente |
